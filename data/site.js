...
/**
 * Update document.title and .navbar-brand text
 * based on the current URL “file name” and an optional override map.
 *
 * @param {Object} [titleMap]  map from page-key → pretty title
 */
function updatePageTitle() {
    // Default titles
    const defaults = {
        index: "Wsprry Pi Configuration",
        view_logs: "Wsprry Pi Logs",
        view_spots: "WSPR Spots",
    };

    // merge defaults + overrides
    const map = Object.assign({}, defaults, titleMap);

    // derive page key from URL
    const path = window.location.pathname;
    const file = path.substring(path.lastIndexOf("/") + 1) || "index";
    const key = file.replace(/\.[^/.]+$/, "") || "index";
    ...
    // update any .navbar-brand text nodes
...
function updateWSPRNetLink() {
  ...
  if ($cs[0].checkValidity() && callsign !== "") {
    ...
    $link.attr("href", WSPRNET_URL).attr("title", "WSPR Spot Database");
